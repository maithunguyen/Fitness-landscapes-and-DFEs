...
for (n in npool) {
  for (dimin in diminpool) {
    ...
    for (i in start_points) {
      fit <- matrix(NA, ncol = n+1, nrow = n)
      gene <- genotype_pool[,1]
      gene <- genotype_pool[,i]
      #calculate fitness of each
      current_fitness <- min(dimin^(sum(gene)-1),1)*(wref+sum(s*gene))
      ...
      neighbors_fitness <- dimincoef*(wref+apply(s*tab,2,sum))
    ...
    saveRDS(biglist, file = paste("dimin_add_","n_", as.character(n),"dimin_", as.character(dimin),".rds", sep = ""))
  }
}
  

saveRDS(pathlength, file = "pathlengths_dimin_add.rds")
