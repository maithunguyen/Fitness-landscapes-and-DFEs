  ...
  for (u in c(1e-3,1e-4,1e-6)) {
    ...
    for (l in 1:repeat_sim) {
      ...
      for (i in 1:generations){
        number_of_mutation <- colSums(data)
        ...
        children_individuals <- wref*(apply((1+s)^data,2,prod))
        ...
        children_individuals <- sapply(children_individuals, function(x) {rpois(1,x)}) #number of children iof each column 
...
file.copy(from=plots.png.paths, to="deminishing return")
