...
npool <- 100
n <- 100
ratiopool <- c(0.1,0.25,0.4,0.7,1,10)
...
for (n in npool) {
  ...
  genotype_pool <- rbind(rep(0,npool),genotype_pool)
  ...
  for (ratio in ratiopool) {  #ok to have genotype pool before loop, genotype pool only change with n.
    sd <- 1/b_lambda/ratio
    noise <- c(0,rnorm(ncol(genotype_pool),0,sd)) #lognormal distribution, noise change for all gene with new ratio loop
    ...
    saveRDS(biglist, file = paste("fuji_add_3_","n_", as.character(n),"ratio_", as.character(ratio),".rds", sep = ""))
  }
}


saveRDS(pathlength, file = "pathlengths_fuji_add_3.rds")
